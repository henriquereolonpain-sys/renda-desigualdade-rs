...
PIB per capita e concentração de renda nas 29 maiores cidades do RS (as 25 mais populosas do estado + 4 do Planalto Médio gaúcho, origem deste projeto), pra mostrar por que média sozinha não conta a história toda sobre riqueza de um município.
...
A renda mais concentrada do estado é a de **Porto Alegre**: em 2022, a renda domiciliar per capita média da capital (R$ 3.203) era **1,88 vezes** a mediana (R$ 1.700), a maior razão média/mediana entre as 29 cidades. No outro extremo, **Alvorada** (razão de 1,25) e as cidades do Planalto Médio como **Serafina Corrêa** e **Tapejara** (1,31) têm a renda mais igualmente distribuída.

PIB per capita também não segue a lógica óbvia de "cidade grande = mais rica per capita": **Santa Cruz do Sul** (R$ 96.885) e **Guaíba** (R$ 93.821) superam a própria Porto Alegre (R$ 75.420), puxadas por indústria concentrada — o mesmo padrão que motivou este projeto lá no início, com Marau superando Passo Fundo.
...
| Bento Gonçalves | 127.980 | 70.812,61 | 2.516,92 | 1.900,00 | 1,33 |
...
| Alvorada | 194.061 | 21.281,63 | 1.332,84 | 1.070,67 | **1,25** |
...
**Por que salário médio e Gini não cobrem as 29**: essas duas fontes não têm uma tabela em lote fácil de consultar por API (o SIDRA não tem "salário médio" sem corte de população em formato tabular direto, e o Gini municipal do Atlas Brasil só existe por página individual). Pra 4 cidades, navegar manualmente é viável; pra 29 (e no futuro, mais), não compensa o esforço frente ao ganho. PIB per capita e renda 2022, por outro lado, têm tabelas SIDRA que aceitam uma lista de códigos de município numa chamada só — por isso são a base da expansão.

**PIB per capita**: o SIDRA não publica um "PIB per capita por município" pronto — o valor exibido no IBGE Cidades é calculado internamente pelo IBGE dividindo o PIB total (tabela 5938) pela população do ano de referência. Este projeto reproduz esse cálculo usando a população estimada mais recente (2026) como denominador, o que gera uma pequena diferença frente ao número oficial do IBGE Cidades nas 4 cidades originais (por exemplo, Passo Fundo: R$ 68.227,53 oficial vs. ~R$ 65.494 recalculado) — por isso essas 4 cidades mantêm o valor oficial do IBGE Cidades na tabela, e as demais 25 usam o valor calculado (coluna `pib_per_capita_fonte` no CSV indica qual é qual).
...
- [x] ~~Expandir pras maiores cidades do RS~~: 25 maiores + as 4 originais do Planalto Médio
- [ ] Restante dos ~468 municípios do RS
