...
**Por que salário médio e Gini não cobrem os 497**: essas duas fontes não têm uma tabela em lote fácil de consultar por API (o SIDRA não tem "salário médio" sem corte de população em formato tabular direto, e o Gini municipal do Atlas Brasil só existe por página individual). Pra 4 cidades, navegar manualmente é viável; pra 497, não compensa o esforço frente ao ganho. PIB per capita e renda 2022, por outro lado, têm tabelas SIDRA que aceitam uma lista de códigos de município numa chamada só — por isso são a base do dataset completo.
...
**Volatilidade em município pequeno**: a razão média/mediana vem de uma amostra do Censo, não de um censo completo de renda. Em município grande (Porto Alegre, Pelotas, Caxias do Sul), a amostra é grande o bastante pra a razão ser um retrato estável da distribuição de renda. Em município de poucos milhares de habitantes, a amostra é pequena, e uma dúzia de domicílios com renda atípica já desloca bastante o valor — é por isso que os dois extremos do ranking completo (Rondinha, Tiradentes do Sul, Novo Machado, Mampituba) são cidades pequenas, não porque tenham necessariamente a distribuição de renda mais extrema do estado, mas porque a amostra ali é mais sensível a ruído estatístico. Pra comparações mais robustas, as cidades grandes (dezenas de milhares de habitantes pra cima) são a referência mais confiável.
